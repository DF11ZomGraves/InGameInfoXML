...
public class Ticker {
	...
	@SubscribeEvent
	public void onRenderGuiOverlayEvent(final RenderGuiLayerEvent.Pre event) {
		...
		else if (overlay == VanillaGuiLayers.EFFECTS && !ConfigurationHandler.showOverlayPotions)
			event.setCanceled(true);
	}
	...
	public void onClientTick(final ClientTickEvent.Pre event) {
		...
		if (inGame) {
			if (!inGameCurrent) {
				...
				if (delay > 1500 || delay < 0) {
					...
					if (singleServer != null) {
						...
						try {
							InGameInfoXML.serverInstalled = false;
							PacketDistributor.sendToServer(new RequestMSPTPacket(0D));
						} catch (Exception e) {
							InGameInfoXML.mspt = -1;
							InGameInfoXML.tps = -1;
						}
			...
			try {
				...
				showVerionInfo();
				PacketDistributor.sendToServer(new RequestSeedPacket(0L));
			} catch (Exception e) {
				InGameInfoXML.seed = ConfigurationHandler.seed;
			}
		if (!enabled || this.client.options == null)
