...
public class InfoIcon extends Info {
	...
	public InfoIcon(final String location, final int displayX, final int displayY, final int displayWidth,
		...
		this(ResourceLocation.parse(location), displayX, displayY, displayWidth, displayHeight, iconX, iconY, iconWidth,
	...
	public void drawInfo(GuiGraphics guiGraphics) {
		...
		try {
			...
			bufferbuilder.addVertex(pose, x1, y1, 0).setUv(this.uv0.x, this.uv0.y);
			bufferbuilder.addVertex(pose, x1, y2, 0).setUv(this.uv0.x, this.uv1.y);
			bufferbuilder.addVertex(pose, x2, y2, 0).setUv(this.uv1.x, this.uv1.y);
			bufferbuilder.addVertex(pose, x2, y1, 0).setUv(this.uv1.x, this.uv0.y);
			BufferUploader.drawWithShader(bufferbuilder.buildOrThrow());
		} catch (final Exception e) {
			InGameInfoXML.logger.debug(e.toString());
		}
	}
