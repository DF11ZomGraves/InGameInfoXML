...
public abstract class TagPlayerEffect extends Tag {
	...
	public static class Effect extends TagPlayerEffect {
		...
		public String getValue() {
			...
			if (potionEffects.length > potionIndex) {
				Holder<MobEffect> effect = potionEffects[potionIndex].getEffect();
				String str = I18n.get(effect.value().getDescriptionId());
	...
	public static class Duration extends TagPlayerEffect {
		...
		public String getValue() {
			...
			if (potionEffects.length > this.index) {
				...
				if (duration == -1)
					return "\u221e";
				return StringUtil.formatTickDuration(duration, minecraft.level.tickRateManager().tickrate());
	...
	public static class Negative extends TagPlayerEffect {
		...
		public String getValue() {
			...
			if (potionEffects.length > this.index) {
				...
				boolean isBadEffect = potionEffects[this.index].getEffect().value().getCategory() == MobEffectCategory.HARMFUL;
	...
	public static class Icon extends TagPlayerEffect {
		...
		public String getValue() {
			updatePotionEffects();
			if (potionEffects.length > this.index) {
				final Holder<MobEffect> mobEffect = potionEffects[this.index].getEffect();
