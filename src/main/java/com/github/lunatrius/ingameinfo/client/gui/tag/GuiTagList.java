...
public class GuiTagList extends GuiListExtended {
	...
	public void filter(final String pattern) {
		...
		for (final Map.Entry<CategoryEntry, Set<TagEntry>> entry : this.map.entrySet()) {
			...
			for (final TagEntry tag : entry.getValue()) {
				if (tag.getName().toLowerCase(Locale.ENGLISH).contains(pattern)
						|| tag.getDesc().toLowerCase(Locale.ENGLISH).contains(pattern)) {
					added = true;
					list.add(tag);
				}
			}
	...
	public abstract class ListEntry implements IGuiListEntry, Comparable<ListEntry> {
		@Override
		public boolean mousePressed(final int index, final int x, final int y, final int mouseEvent,
				final int relativeX, final int relativeY) {
			return false;
		}

		@Override
		public void mouseReleased(final int index, final int x, final int y, final int mouseEvent, final int relativeX,
				final int relativeY) {
		}

		@Override
		public void updatePosition(final int p_192633_1_, final int p_192633_2_, final int p_192633_3_,
				final float p_192633_4_) {
		}
	...
	public class CategoryEntry extends ListEntry {
		...
		public void drawEntry(final int slotIndex, final int x, final int y, final int listWidth, final int slotHeight,
			...
			this.fontRenderer.drawString(this.name, x + (listWidth - this.fontRenderer.getStringWidth(this.name)) / 2,
					y + (slotHeight - this.fontRenderer.FONT_HEIGHT + 1) / 2, 0xFFFFFF);
		}
	...
	public class TagEntry extends ListEntry {
		...
		public void drawEntry(final int slotIndex, final int x, final int y, final int listWidth, final int slotHeight,
				final int mouseX, final int mouseY, final boolean isSelected, final float partialTicks) {
			this.fontRenderer.drawString(this.name, x, y + slotHeight / 2 - this.fontRenderer.FONT_HEIGHT / 2,
					0xFFFFFF);

			final int lineHeight = this.fontRenderer.FONT_HEIGHT + 1;
			for (int i = 0; i < this.descArray.length; i++) {
				this.fontRenderer.drawString(this.descArray[i], x + OFFSET_X,
						y + (slotHeight - lineHeight * this.descArray.length) / 2 + lineHeight * i, 0xFFFFFF);
			}
		}
