...
public abstract class TagPlayerPosition extends Tag {
	...
	public static class Speed extends TagPlayerPosition {
		@Override
		public String getValue() {
			return String.format(Locale.ENGLISH, "%.3f", TICKS * Math.sqrt(playerMotion.x * playerMotion.x
					+ playerMotion.y * playerMotion.y + playerMotion.z * playerMotion.z));
		}
	}
	...
	public static class SpeedXZ extends TagPlayerPosition {
		@Override
		public String getValue() {
			return String.format(Locale.ENGLISH, "%.3f",
	...
	public static class DirectionHud extends TagPlayerPosition {
		...
		public String getValue() {
			...
			final String left = ABRFINEDIRECTION[(direction / 2 + ABRFINEDIRECTION.length - 1)
					% ABRFINEDIRECTION.length];
			final String center = ABRFINEDIRECTION[(direction / 2 + ABRFINEDIRECTION.length) % ABRFINEDIRECTION.length];
			final String right = ABRFINEDIRECTION[(direction / 2 + ABRFINEDIRECTION.length + 1)
					% ABRFINEDIRECTION.length];
			if (direction % 2 == 0)
